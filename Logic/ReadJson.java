...
//metodo sobrecargado
    ...
    //metodo que lee el archivo .json
    ...
    public void createNodes() {
        ...
        for (Origen user : OriginsArray) {
            ...
            for (int i = 0; i < user.getTargets().size(); i++) {
                Node<Destination> destinoNode = new Node<Destination>(new Destination(user.getTargets().get(i).getTarget(), user.getTargets().get(i).getDistance(), user.getTargets().get(i).getValue()), user.getTargets().get(i).getTarget());
...
//        System.out.println("REadSon" + arrayListOrigen);
    }
